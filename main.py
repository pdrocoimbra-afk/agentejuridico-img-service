...
def extract_tema(estrategista_output: str) -> str:
    for part in estrategista_output.split("|"):
        ...
        if upper.startswith("TEMA:"):
            return part[5:].strip()[:25].rstrip().upper()
        if upper.startswith("TOPICO:"):
            return part[part.index(":") + 1:].strip()[:25].rstrip().upper()
...
def watermark(draw, w=1080, h=1080):
    ...
    draw.text((w // 2, h - 28), "@agentejuridico", font=font, fill=(*GOLD, 160), anchor="mb")
...
def compose_alerta(req: AlertaRequest):
    ...
    box_h = 160
    draw.rounded_rectangle([(60, y), (W - 60, y + box_h)], radius=12, fill=(35, 5, 5), outline=(110, 25, 25), width=2)
...
def compose_auto(req: AutoRequest):
    ...
    if formato == "dado":
        ...
        reacao = extract_field(req.estrategista_output, "EXTRA2") or "SERIO MESMO?"
        ...
        if not req.image_url:
            raise HTTPException(status_code=400, detail="image_url obrigatorio para formato padrao")
